...
class MakeServerCommand extends VanillaCommand
{
	...
	public function execute(CommandSender $sender, $commandLabel, array $args)
	{
		...
		if(file_exists($pharPath)) {
			$sender->sendMessage("[NekoMine] " . "Phar file already exists, overwriting...");
			@unlink($pharPath);
		}
		...
		$phar->stopBuffering();
		$sender->sendMessage("[NekoMine] " . $server->getName() . " " . $server->getPocketMineVersion() . " Phar file has been created on " . $pharPath);
		return true;
	}
}
