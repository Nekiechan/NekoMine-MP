...
class NoticeCommand extends VanillaCommand{
	public function __construct($name){
		parent::__construct(
			$name,
      "Broadcast a notice across the server!",
			"/notice <msg>"
		);
		$this->setPermission("pocketmine.command.notice");
	}
    ...
    if(count($args) === 0){
      $sender->sendMessage("§l§cPut a Msg! §a/notice <msg>");
      return false;
    }
		$sender->getServer()->broadcastMessage(TextFormat::GREEN . "[NOTICE] " . implode(" ", $args));
		return true;
    }
}
