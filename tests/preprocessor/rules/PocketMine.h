...
#define Level::blockHash(x, y, z) (PHP_INT_SIZE === 8 ? (((x) & 0xFFFFFFF) << 36) | (((y) & 0xff) << 28) | ((z) & 0xFFFFFFF) : (x) . ":" . (y) .":". (z))
#define Level::getXZ(hash, chunkX, chunkZ) if(PHP_INT_SIZE === 8){chunkX = (hash >> 32); chunkZ = (hash & 0xFFFFFFFF) << 32 >> 32;}else{list(chunkX, chunkZ) = explode(":", hash); chunkX = (int) chunkX; chunkZ = (int) chunkZ;}
#define Level::getBlockXYZ(hash, X, Y, Z) if(PHP_INT_SIZE === 8){X = (hash >> 36); Y = ((hash >> 28) & 0xff); Z = (hash & 0xFFFFFFF) << 36 >> 36;}else{list(X, Y, Z) = explode(":", hash); X = (int) X; Y = (int) Y; Z = (int) Z;}
#endif
#endif
